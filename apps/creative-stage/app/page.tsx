  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    if (text.trim() === "/help" || text.trim() === "/?") {
      chat.getState().addMessage([
        ...
        "  /demo jazz sunset  Generates everything from 2 words\n",
        "STREAM",
        "  \"start a dreamy sunset\"    Start live stream",
        "  \"change to anime style\"    Update prompt mid-stream",
        "  \"add 4 scenes\"             Scene sequence + auto-play",
        "  Space                       Play/stop performance",
        "  R                           Record stream\n",
        "CANVAS",
        "  Drag image → stream         Auto-analyze + set as source",
        "  Clear (source badge)         Reset to scene prompt",
        "  Double-click image           Fullscreen view",
        "  Right-click card             Animate, edit, delete",
        "  ✕ on card                    Remove",
        "  Cmd+Z                        Undo\n",
        "CREATE",
        "  \"generate an image of...\"   Image on canvas",
        "  \"create a video of...\"      Video clip",
        "  \"generate music: calm jazz\" Music + beat sync",
        "  \"compare gpt and flux\"      Side-by-side models\n",
        "TIPS",
        "  • Gear icon → set API key first",
        "  • noise_scale 0.2=faithful, 0.8=creative",
        "  • Scenes auto-transition on Space",
      ].join("\n"), "agent");
      return;
    }
